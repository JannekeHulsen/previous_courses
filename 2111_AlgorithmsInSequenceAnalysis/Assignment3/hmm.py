...
def forward(X,A,E):
    ...
    # Last columns
    terms = [F[k][i+1] * A[k]['E'] for k in allStates]
    F['E'][-1] = sum(terms)
...
def backward(X,A,E):
    ...
    for i in range(L-3,-1,-1):
        s = X[i]
        for k in allStates:
            terms = [A[k][l] * B[l][i+1] * E[l][s] for l in emittingStates]
            B[k][i] = sum(terms)
...
def baumwelch(set_X,A,E):
    ...
    for X in set_X:
        ...
        for i in range(len(X)):
            # Emissions.
            for k in emittingStates:
                new_E[k][X[i]] += 1/P * F[k][i+1] * B[k][i+1]

            # Transitions.
            for k in allStates:
                for l in allStates:
                    if l in emittingStates:
                        new_A[k][l] += 1/P * F[k][i] * A[k][l] * E[l][X[i]] * B[l][i+1]
                    elif l == "E" and i == len(X)-1:
                        new_A[k][l] += 1/P * F[k][i+1] * A[k][l]
    ...
    for k in allStates:
        if k != "E":
            rowsums[k] = sum([new_A[k][l] for l in allStates])
            for l in allStates:
                new_A[k][l] = new_A[k][l]/rowsums[k]

    # Emissions.
    rowsums = dict()
    for k in emittingStates:
        rowsums[k] = sum([new_E[k][s] for s in dict.keys(new_E[k])])
        for s in dict.keys(new_E[k]):
            new_E[k][s] = new_E[k][s]/rowsums[k]
